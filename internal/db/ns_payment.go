...
func (db *clietn) PaymentCreate(p *Payment) (int64, error) {
	...
	row := db.QueryRow(context.TODO(), query, p.Doctype, p.CreatedAt, p.OrganizationID, p.CounterpartyID, p.IncomingCurrency, p.ExpendableCurrency, p.Purpose, p.Expenditure, p.Comments)
	err := row.Scan(&id)
	if err != nil {
